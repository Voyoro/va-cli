import { color } from '@ebdp-script/node-va-utils'
...
function highlight(content: string, indentSize: number = 2, highlight: boolean = false): string {
  ...
  const ansi = highlight ? color.cyan : color.reset
  ...
  if (indentLevel === 0 || indentLevel === 1) {
    ...
    if (versionMatch && versionMatch[1].trim())
      return leadingSpaces + ansi(propertyName) + color.dim(afterColon)
    else
      return leadingSpaces + ansi(propertyName) + color.dim(':')
  }
  else {
    const versionMatch = afterColon.match(/:\s*(.+)/)
    if (versionMatch && versionMatch[1].trim())
      return beforeColon + color.dim(afterColon)
    else
      return beforeColon + color.dim(':')
  }
}
...
export function diffHighlight(original: string, updated: string, options: DiffHighlightOptions = {}) {
  ...
  if (addedCount > 0) {
    summaryParts.push(`${color.yellow(addedCount)} added`)
  }
  if (removedCount > 0) {
    summaryParts.push(`${color.yellow(removedCount)} removed`)
  }
  ...
  diffs.forEach((line, index) => {
    if (linesToKeep.has(index)) {
      if (!verbose && lastKeptIndex !== -1 && index > lastKeptIndex + 1) {
        ...
        result.push(color.dim`${color.yellow(skippedCount)} unchanged line${skippedCount > 1 ? 's' : ''}`)
      }

      let coloredLine = line.content
      if (line.type === 'added') {
        const highlighted = highlight(line.content, indentSize, verbose)
        coloredLine = color.green(`+ ${highlighted}`)
      }
      else if (line.type === 'removed') {
        const highlighted = highlight(line.content, indentSize, verbose)
        coloredLine = color.red(`- ${highlighted}`)
      }
      else {
        const highlighted = highlight(line.content, indentSize, verbose)
