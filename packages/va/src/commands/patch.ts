import { gitBranch, gitHead, gitStatus } from '@ebdp-script/node-va-utils'
...
export function definePatchCommand(cac: CAC) {
  cac.command('patch', 'Generate local patches based on the current local modifications')
    .action(async () => {
      ...
      const outDir = path.join(cwd, 'dev-dist')
      fs.mkdirSync(outDir, { recursive: true })
      const outPath = path.join(outDir, outName)
      zip.writeZip(outPath)

      console.log('Patch export completed:', outName)
      console.log('Included files:', manifest.files.length)
      console.log('Deleted files:', manifest.deleted.length)
      console.log('Apply this patch with the apply command.')
    })
}
